...
void setup() {
    Serial.begin(115200);
    initMotors();
    WiFi.setHostname("perlin-v4"); WiFi.begin(DEFAULT_SSID, DEFAULT_PASS);
    unsigned long sW = millis(); while (WiFi.status() != WL_CONNECTED && millis()-sW < 8000) { delay(500); }
    if (WiFi.status() != WL_CONNECTED) { WiFi.mode(WIFI_AP); WiFi.softAP("perlin-v4-setup"); } // kein Default-Passwort im AP-Mode
    else { MDNS.begin("perlin-v4"); }
    
    ArduinoOTA.setHostname("perlin-v4"); ArduinoOTA.begin();
    ...
    server.on("/status", [](AsyncWebServerRequest *r){
        ...
        uint8_t opIdx = sys.opState < 6 ? sys.opState : 0; // W9-Fix: Bounds-Check vor opNames-Zugriff
        String j = "{\"hit\":" + String(digitalRead(TACHO_PIN)==LOW?"true":"false");
        j += ",\"op\":\"" + String(opNames[opIdx]) + "\"";
