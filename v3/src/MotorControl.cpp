...
void learnSGProfile(int i) {
    ...
    for(int s=0; s<4; s++) {
        ...
        while(steppers[0]->distanceToGo() != 0) {
            steppers[0]->run();
            if(abs(steppers[0]->speed()) > sps*0.8f) { sgSum += (float)driverX.SG_RESULT(); samples++; }
            yield();
        }
    }
