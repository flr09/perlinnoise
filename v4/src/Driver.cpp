...
void applyDriverSettings(uint16_t runMA) {
    if (xSemaphoreTake(uartMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
        ...
        driverX.TPWMTHRS(rpmToTpwmthrs(200)); // O4-Fix: 200 RPM (vorher 100) — stabiler SpreadCycle-Uebergang, kein Hunting
        driverX.en_spreadCycle(false);
        driverX.TCOOLTHRS(rpmToTpwmthrs(100)); // CoolStep/SG ab 100 RPM aktiv
...
void setMicrosteps(uint16_t ms) {
    ...
    // W5-Fix: TMC zuerst aktualisieren, dann FAS-Position skalieren.
    // Vorher war die Reihenfolge umgekehrt: stepsPerRev war neu, TMC noch alt —
    // im Race-Fenster haetten Schritte in falschem Microstep-Modus interpretiert werden koennen.
    if (xSemaphoreTake(uartMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
...
float getPositionDeg()          { return stepper ? stepsToDeg(stepper->getCurrentPosition()) : NAN; } // W6-Fix: NAN statt 0.0f — Caller kann NULL-Stepper erkennen
