...
| 33 | 2026-05-06 | 🔴 | TMC2209 StallGuard liefert bei oszillierender Bewegung konstant `SG_RESULT=0` selbst bei nachweislicher Motor-Bewegung (Tacho zeigt Pulse). Ursache: SG braucht continuous spinning über TPWMTHRS-Schwelle, Reversal-Halbperioden 50 ms zu kurz. Plus Telemetry-Poll 100 ms verpasst Spitzen. | 🔴 noted (alternative Stall-Detection für Reversal nicht möglich ohne SpreadCycle-Umstellung + UART-Latenz-Optimierung). Research-Notiz von Gemini: `v4/docs/research_stallguard.md` — SG4 unter StealthChop wäre die richtige Wahl für künftige v5. |
| 34 | 2026-05-06 | 🔴 | Sinus-Mode mechanisch laut: `applyEngineCap` setzt für alle Wave-Modi (3-5) pauschal accCap = `cal.maxAccel × 0.95` (475 k sps² für Z) — für Square nötig, für Sinus/Saw völlig überdimensioniert → Klacken statt sanftem Schwingen. Verstößt gegen den künstlerischen Anspruch „silent" für Sinus. | ✅ v4.2.4 (per-Wave-Mode-Discrimination: Sinus = DEFAULT_ACC_NOISE 4000, Saw = 50 k, Square = volle cal.maxAccel) |
