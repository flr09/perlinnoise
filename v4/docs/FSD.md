...
| 4.2.0 | 2026-05-06 | Claude | WiFi-Credentials in NVS (Bug-ID 23c). Neuer L2-Block `Storage_Wifi` (NVS-Section `wifi`). `Wifi::connectOrAP()` lädt zuerst NVS, fällt auf `wifi_settings.h` zurück wenn leer. Neue Endpoints `/wifisave?s=…&p=…` und `/wificlear` mit **deferred Reboot** (Flag + 1.5 s im main-loop, KEIN delay() im Handler — vermeidet Geminis Bug aus 8c0f389). Hardware-verifiziert: Round-Trip mit produktiven Creds, Board kommt nach Reboot auf NVS-Pfad zurück. Persistence-Familie damit komplett. |
