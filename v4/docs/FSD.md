...
| **D** | Player **responsive**: Routen-Split ist bereits da (`/` Player + `/test` Tests seit Phase 6). Bei `<600 px` neue Media-Query: Motor-, Spatial- und Log-Sektionen versteckt via `section:has(>.mg/.cg/.log)`. Performance-Sektion (Pattern + Slider + Start/Stop + REC + Presets) bleibt sichtbar mit kompakteren Rows + größeren Touch-Targets. Status-Strip + Canvas + Dials bleiben. | v4.4.7 | ✅ 2026-05-13 |
...
| **I** | ~~Chart.js lokal einbetten~~ — **deferred**. Chart.js v4.x min ≈ 200 KB würde Flash von 1.03 → 1.23 MB drücken (Partition-Limit 1.31 MB), ist riskant. Lab-Seite hat aktuell **keine** Live-Diagramme, die geplottet werden müssten — die fc-Tabellen in der FSD sind statisch. Wenn echter Chart-Bedarf entsteht (z.B. RPM-History, fc-Sweep-Resultate live), kommt eine schlanke Custom-Canvas-Lösung statt Chart.js. | — | ⏸ deferred |
...
| 4.4.7 | 2026-05-13 | Claude | Phase 10 Schritt D (Player responsive, App-Variante) + Schritt I deferred. **D**: neue Media-Query `@media(max-width:599px)` in INDEX_HTML. Bei kleiner Viewport-Breite werden Motor-Sektion (Calib/Home/Zero), Spatial-Compass-Cards (zu fummelig auf Touch) und Log-Sektion versteckt — via `section:has(>.mg/.cg/.log){display:none}` (CSS-`:has()` ab Chrome 105/Safari 15.4). Sichtbar bleibt: Header, Status-Strip, Noise-Canvas + Dials, Performance-Sektion mit Pattern/Slider/Start/Stop/REC/Presets. Slider-Rows kompakter (Margin 8→6 px, Range-Slider-Höhe 24→32 px für bessere Touch-Targets), Stop/Start-Buttons auf 52 px Höhe, Footer-Text reduziert. **I**: Chart.js nicht eingebettet (200 KB würde Flash 1.03→1.23 MB drücken, Partition-Limit 1.31 MB knapp). Lab-Seite hat aktuell keine konkreten Chart-Daten. Bei echtem Bedarf später schlanke Custom-Canvas-Sparklines. Hardware: v4.4.7 OTA mit `Connection: close` sauber, HTML enthält die neuen Media-Query-Regeln. |
